<?php
class Createblog extends JP_Action implements JXP_Action{
    private $passed, $nameError,$urlError,$contentError,$stepper=[], $summaryError,$logoError,$catError;
    public function __construct()
    {
      $this->setTitle("Add New Blog");
      $this->setText("Add new Blog to your Account");
      $this->passed = false;
      $this->stepper = [
          'empty' => false,
          'url' => false,
    ...
    private function checkcat(){
     ...
     private function process(){
         global $JX_db, $BLOG_OWNER,$BLOG_AUTHOR;;
            $name = $_POST['title'];
            ...
            if($logo !== null){
                $sql = "INSERT INTO `blogs`(`name`,`type`,`category`,`owner`,`author`,`description`,`summary`,`keywords`,`theme`,`url`,`logo`)VALUES('$name','blog','$cat','$owner','$author','$content','$summary','$keywords','Default','$url','$logo')";
                if($JX_db->query($sql)){
                    JP_Alert("Blog Alert","Your Blog was Successfully Created","light-blue");
                }else{
                    JP_Alert("Blog Alert","We cannot create your blog at the right moment please try again later","light-blue");
                }
            }
     }
