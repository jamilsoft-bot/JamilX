...
class UpdateCat extends JP_Action implements JXP_Action{
    ...
    public function getAction(){
        if(isset($_GET['id'])){
            $this->getData();
            $data = new JP_Cat($_GET['id']);
        echo "<form action='' enctype='multipart/form-data' method='post'>";
        include "Apps/jamilpress/containers/post-add.php";
        echo "</form>";
        }else{
            JP_Alert("Blog alert","Please Select Post To update","red");
        }
     }
    ...
    private function checkcat(){
     ...
     private function checkempty(){
            ...
            if(empty($_POST['content'])){
                $this->contentError = "This is Field cannot be Empty";
                $this->passed = false;
     ...
     private function process(){
         global $JX_db, $BLOG_OWNER,$BLOG_AUTHOR, $BLOG_URL;
            ...
            if($logo !== null){
                $sql = "UPDATE `categories` SET `parent`='$cat', `status`='$status', `privacy`='$privacy', `keywords`='$keywords',`summary`='$summary', `name`='$name', `description`='$content', `image`='$logo' WHERE `id`=$id";
                if($JX_db->query($sql)){
                    JP_Alert("Blog Alert","Your Category was Successfully Updated","light-blue");
                }else{
                    JP_Alert("Blog Alert","We cannot Update your Category at the right moment please try again later","light-blue");
                }
            }
     }
